...
export const TermsAndCondition: React.FC<TermsAndConditionProps> = ({ token }) => {
  ...
  const fetchAbout = useCallback(async () => {
    setLoading(true);
    try {
      const res = await apiRequest(ENDPOINTS.TERMSANDCONDITIONS.LIST, 'GET', null, token);
      if (res.status && res.data && res.data.terms) {
        setAboutData(res.data.terms);
      }
    } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await apiRequest(ENDPOINTS.TERMSANDCONDITIONS.CREATE, 'POST', formData, token, true);
      if (res.status) {
        ...
        <div className="bg-white rounded-3xl shadow-2xl overflow-hidden border border-gray-100">
          <div className="px-8 py-6 bg-indigo-900 flex justify-between items-center text-white">
            <div className="flex items-center space-x-4">
              ...
              <h2 className="text-2xl font-bold">Manage Terms and Conditions Content</h2>
            </div>
  ...
  return (
    <div className="max-w-7xl mx-auto py-10 px-6 space-y-10 animate-in fade-in duration-500">
      ...
      <div className="flex justify-between items-end">
        <div>
          <h2 className="text-5xl font-black text-gray-900 tracking-tight">Terms and Conditions</h2>
        ...
        {!loading && !aboutData && (
          <Button onClick={() => { resetForm(); setView('create'); }} className="px-8 py-3 rounded-2xl shadow-lg">
            <Plus className="w-5 h-5 mr-2" /> Create Terms and Conditions Content
          </Button>
        )}
      </div>
      ...
      <div className="grid grid-cols-1 gap-10">
        {loading ? (
          ...
          <div className="py-24 text-center bg-white rounded-[40px] border-2 border-dashed border-gray-100">
            ...
            <p className="text-gray-500 text-lg mb-8">Start by creating the initial Terms and Conditions content.</p>
